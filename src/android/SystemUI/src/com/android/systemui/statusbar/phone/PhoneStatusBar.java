...
import com.android.systemui.statusbar.policy.DcloudController;
...
public class PhoneStatusBar extends BaseStatusBar {
    ...
    DcloudController mDcloudController;
	...
	// DCloud 버튼
	dcloudButton = mStatusBarWindow.findViewById(R.id.dcloud_settings_button);
        dcloudButton.setOnClickListener(dcloudButtonListener);
        dcloudButton.setOnLongClickListener(dcloudButtonListener2);

	// DCloud On/OFF 상태 판단
	boolean c = Settings.System.getInt(mContext.getContentResolver(), "DCLOUD_MODE_ON", 0) != 0;
	if(c) {
		((ImageView)dcloudButton).setImageResource(R.drawable.ic_notify_quickdcloud_running);
	}
        ...
        mBatteryController.addIconView((ImageView)mStatusBarView.findViewById(R.id.battery));
         ...
         // Dcloud Icon 추가
        mDcloudController = new DcloudController(mContext);
        mDcloudController.addIconView((ImageView)mStatusBarView.findViewById(R.id.dcloud));
...
//        }
	...
	dcloudButton.setEnabled(isDeviceProvisioned());
    }
    ...
    boolean interceptTouchEvent(MotionEvent event) {
	    ...
	    // Dark Cloud ON/OFF 확인	    
	    boolean c = Settings.System.getInt(mContext.getContentResolver(), "DCLOUD_MODE_ON", 0) != 0;
	    if(c) ((ImageView)dcloudButton).setImageResource(R.drawable.ic_notify_quickdcloud_running);
	    else  ((ImageView)dcloudButton).setImageResource(R.drawable.ic_notify_quickdcloud);
    ...
    private View.OnClickListener dcloudButtonListener = new View.OnClickListener() {
        public void onClick(View v) {
            if (!isDeviceProvisioned()) return;
            try {
                ActivityManagerNative.getDefault().dismissKeyguardOnNextActivity();
         ...
         // Dcloud 서비스 메세지 전달
		  ...
		  // DCloud On/OFF 상태 판단
		  boolean c = Settings.System.getInt(mContext.getContentResolver(), "DCLOUD_MODE_ON", 0) != 0;
		  if(c) i.putExtra("MSG", "0");
		  else  i.putExtra("MSG", "1");
		  mContext.startService(i);
		  
	    //Intent intent = new Intent();
	    //intent.setComponent(new ComponentName("com.jk.postpic", "com.jk.postpic.ui.Intro"));
	    //v.getContext().startActivity(intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK));
	    //Intent intent = new Intent(Intent.ACTION_DIAL, Uri.parse("tel:01026620890"));
	    //v.getContext().startActivity(intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK));
	    ...
	    //Toast.makeText(mContext, "DeviceId:" + deviceid, Toast.LENGTH_LONG).show();
        }
    };
    
    private View.OnLongClickListener dcloudButtonListener2 = new View.OnLongClickListener() {
		public boolean onLongClick(View arg0) {
			GCMRegistrar.unregister(SystemUI.mContext.getApplicationContext());
        	GCMRegistrar.checkDevice(SystemUI.mContext.getApplicationContext());
    		GCMRegistrar.checkManifest(SystemUI.mContext.getApplicationContext());
    		
    		final String regId = GCMRegistrar.getRegistrationId(SystemUI.mContext.getApplicationContext());
    		if("".equals(regId)) {
    			GCMRegistrar.register(SystemUI.mContext.getApplicationContext(), "796582874100");
    		}
			return false;
		}
	};
