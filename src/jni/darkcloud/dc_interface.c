...
const char opendir_uri[] = "http://211.189.20.179:3000/opendir";
const char create_uri[] = "http://211.189.20.179:3000/create";
const char remove_uri[] = "http://211.189.20.179:3000/remove";
const char read_uri[] = "http://211.189.20.179:3000/read";
const char write_uri[] = "http://211.189.20.179:3000/write";
const char write2_uri[] = "http://211.189.20.138:5000";
const char mkdir_uri[] = "http://211.189.20.179:3000/mkdir";
const char rmdir_uri[] = "http://211.189.20.179:3000/rmdir";
const char access_uri[] = "http://211.189.20.179:3000/access";

void dci_print_all(struct dci_state *state)
{
	char tmp[1024];
	
	fprintf(stderr, "Jake, START: dci_print_all \n");
	...
	fprintf(stderr, "Jake, END: dci_print_all \n");
}
...
void dci_remove_path_data(const char *path, struct dci_state *state)
{
	...
	fprintf(stderr, "Jake, START: dci_remove_path_data %s \n", path);
	struct dc_entry_t* iter = state->head;
	while(iter != NULL)
	{
	...
	fprintf(stderr, "Jake, END: dci_remove_path_data %s \n", path);
}
...
{
	fprintf(stderr, "Jake, START: dci_remove_path_data %s %s \n", item->path.str, item->fileName.str);
	struct dc_entry_t* iter = state->head;
...
{
	fprintf(stderr, "Jake, START: dci_is_already_path %s \n", path);
	...
	fprintf(stderr, "Jake, END: dci_is_already_path NO");
	return FALSE;
}

void dci_destroy(struct dci_state *state)
{
	printf("Jake, Clean up DCI...\n");
	fflush(stdout);
	dci_init_list(state);
...
void dci_opendir(const char *path, struct dci_state *state)
{
	...
	*/
	...
	struct str_t uri;
	str_init_create(&uri, opendir_uri, 0);
...
{
	fprintf(stderr, "Jake, START: dci_mkdir %s %s\n", path, fileName);
	...
	str_init_create(&uri, mkdir_uri, 0);
	str_init(&param_uri);
...
int dci_rmdir(const char *path, struct dci_state *state)
{
	...
	fprintf(stderr, "Jake, END: dci_rmdir\n");
	return 0;
}

int dci_load(struct dci_state* state, struct dc_entry_t* entry)
{
	fprintf(stderr, "Jake, START: dci_load, cached[%d]\n", entry->cached);
	if(entry->cached == 0)
	{
		struct str_t uri;
		str_init_create(&uri, read_uri, 0);
	...
	fprintf(stderr, "Jake, END: dci_load\n");
...
{
	fprintf(stderr, "Jake, START: dci_create %s %s\n", path, fileName);
	struct str_t header ,fileName_param, fileSize_param, uploadDate_param, uploadDate_value;
	...
	struct str_t uri;
	struct str_t param_uri;
	str_init_create(&uri, create_uri, 0);
	str_init(&param_uri);
	...
	fprintf(stderr, "Jake, END: dci_create\n");
	return 0;
}

int dci_unlink(const char *path, struct dci_state *state)
{
	fprintf(stderr, "Jake, START: dci_unlink %s \n", path);
	struct dc_entry_t* entry = find_entry_from_path(state, path);

	struct str_t uri;
	str_init_create(&uri, remove_uri, 0);
	...
	fprintf(stderr, "Jake, END: dci_unlink\n");
	return 0;
}
...
{
	fprintf(stderr, "Jake, START: dci_write %s %s\n", path, fileName);
	...
	struct str_t uri;
	str_init_create(&uri, write_uri, 0);
	...
	free(bodyline);
	fprintf(stderr, "Jake, END: dci_write\n");
	return size;
}
